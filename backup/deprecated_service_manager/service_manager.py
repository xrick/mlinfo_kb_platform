...
class ServiceManager:
    # Services to skip during auto-discovery (deprecated or incompatible)
    SKIP_SERVICES = {'sales_assistant'}  # Deprecated: Uses old multichat module
    ...
    def _discover_services(self, service_directory):
        ...
        for service_name in os.listdir(service_directory):
            ...
            # Skip deprecated or incompatible services
            if service_name in self.SKIP_SERVICES:
                print(f"跳過已棄用的服務: {service_name}")
                continue
