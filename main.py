...
try:
    # 導入活躍的 API 路由模組
    from api import mgfd_routes, specs_routes, history_routes, milvus_routes

    # 將各個路由模組註冊到主應用程式中
    app.include_router(mgfd_routes.router, prefix="/api/mgfd", tags=["mgfd"])  # MGFD 核心系統
    app.include_router(specs_routes.router, prefix="/api/specs", tags=["specs"])  # 規格管理 API
    app.include_router(history_routes.router, prefix="/api/history", tags=["history"])  # 歷史記錄 API
    app.include_router(milvus_routes.router, prefix="/api/milvus", tags=["milvus"])  # Milvus Viewer API
...
async def system_status():
    ...
    try:
        ...
        from api.mgfd_routes import mgfd_system
        if mgfd_system:
            mgfd_status = await mgfd_system.get_system_status()
        else:
